#' \code{fetchModuGrinnNetwork} compute a network module correlated to a phenotypic feature and expand the network with information from Grinn internal database
...
#'\code{blockwiseModules}, \code{WGCNA::cor}, \code{WGCNA::corPvalueStudent}, \code{labeledHeatmap}.
...
#'A module or the combination of modules can be selected from the heatmap of module-phenotype correlations for including in the network output, see more details in \code{fetchWGCNAodule}.
...
#'The column names of datNorm are required to use grinn ids. \code{convertToGrinnID} is provided for id conversion, see \code{convertToGrinnID}.
...
#'@export
#'@seealso \code{\link{WGCNA::cor}}, \code{\link{WGCNA::corPvalueStudent}}, \code{\link{pickSoftThreshold}}, \code{\link{blockwiseModules}}, \code{\link{labeledHeatmap}}, 
...
#'load("data/dummy.RData")
#'load("data/dummyPheno.RData")
