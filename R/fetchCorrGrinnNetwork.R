#' \code{fetchCorrGrinnNetwork} compute a weighted correlation network and expand the network with information from Grinn internal database
#'@description from input omics data e.g. normalized expression data or metabolomics data, it is a one step function to:
#'1. Compute a weighted correlation network of input omics data using WGCNA functions \code{WGCNA::cor} and \code{WGCNA::corPvalueStudent}. 
